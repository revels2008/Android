...
public class BaseDaoImpl<T> implements BaseDao<T> {
	...
	public BaseDaoImpl(SQLiteOpenHelper dbHelper, Class<T> clazz) {
		...
		//....d(TAG, "clazz:" + this.clazz + " tableName:" + this.tableName + " idColumn:" + this.idColumn);
	}
	...
	public T get(int id) {
		...
		//....d(TAG, "[get]: select * from " + this.tableName + " where " + this.idColumn + " = '" + id + "'");
	...
	public List<T> rawQuery(String sql, String[] selectionArgs) {
		//....d(TAG, "[rawQuery]: " + getLogSql(sql, selectionArgs));
		...
		try {
			...
			//....e(this.TAG, "[rawQuery] from DB Exception.");
			e.printStackTrace();
		} finally {
			if (cursor != null) {
				cursor.close();
	...
	public boolean isExist(String sql, String[] selectionArgs) {
		//....d(TAG, "[isExist]: " + getLogSql(sql, selectionArgs));
		...
		try {
			...
			//....e(this.TAG, "[isExist] from DB Exception.");
			e.printStackTrace();
		} finally {
			if (cursor != null) {
				cursor.close();
	...
	public ArrayList<T> find(String[] columns, String selection,
...
//			LogUtil.d("", selectionArgs[0]);
//			LogUtil.d("", Constants.mallId+"");
//			cursor = db.rawQuery("select * from " + this.tableName + "where" + selection + "= ? and mall = ?" , new String[]{selectionArgs[0], Constants.mallId+""});
			getListFromCursor(list, cursor);
		} catch (Exception e) {
			//....e(this.TAG, "[find] from DB Exception");
			e.printStackTrace();
		} finally {
			if (cursor != null) {
				cursor.close();
	...
	public long insert(T entity, boolean flag) {
		...
		try {
			...
			//....d(TAG, "[insert]: insert into " + this.tableName + " " + sql);
			long row = db.insert(this.tableName, null, cv);
			return row;
		} catch (Exception e) {
			//....d(this.TAG, "[insert] into DB Exception.");
			e.printStackTrace();
		} finally {
			if (db != null) {
				db.close();
	 ...
	 * 添加多个
	 */
	public void insertAll(ArrayList<T> entitys)
	{
		for (T t : entitys)
		{
			insert(t);
	...
	public void delete(int id)
	{
		...
		// //....d(TAG, "[delete]: delelte from " + this.tableName + " where "
	...
	public void delete(Integer... ids)
	{
		if (ids.length > 0)
		{
			...
			// //....d(TAG, "[delete]: " + getLogSql(sql, ids));
	...
	public void deleteAll()
	{
		...
		// //....d(TAG, "[delete]: " + getLogSql(sql, null));
	...
	{
		SQLiteDatabase db = this.dbHelper.getWritableDatabase();
		// String sql = "delete from " + this.tableName + " where " + fieldName
		// + " = " + fieldValue;
		String where = fieldName + " = ?";
		String[] whereValue = { fieldValue };
		// //....d(TAG, "[delete]: " + getLogSql(sql, null));
		db.delete(this.tableName, where, whereValue);
		db.close();
	}
	 ...
	 * @param 字段值
	 */
	public void deleteTwoField(String fieldName1, String fieldValue1,
			String fieldName2, String fieldValue2)
	{
		SQLiteDatabase db = this.dbHelper.getWritableDatabase();
		// String sql = "delete from " + this.tableName + " where " + fieldName1
		// + " = " + fieldValue1 + " AND " + fieldName2 + " = " + fieldValue2;
		String where = fieldName1 + " = ? AND " + fieldName2 + " = ?";
		String[] whereValue = { fieldValue1, fieldValue2 };
		// //....d(TAG, "[delete]: " + getLogSql(sql, null));
		db.delete(this.tableName, where, whereValue);
		db.close();
	}
	...
	public void update(T entity) {
		...
		try {
			...
			//....d(TAG, "[update]: update " + this.tableName + " set " + sql + " where " + where.replace("?", String.valueOf(id)));

			String[] whereValue = { Integer.toString(id) };
			db.update(this.tableName, cv, where, whereValue);
		} catch (Exception e) {
			//....d(this.TAG, "[update] DB Exception.");
			e.printStackTrace();
		} finally {
			if (db != null)
				db.close();
	...
	public List<Map<String, String>> query2MapList(String sql,
		...
		//....d(TAG, "[query2MapList]: " + getLogSql(sql, selectionArgs));
		SQLiteDatabase db = null;
		Cursor cursor = null;
		...
		try {
			...
			//....e(TAG, "[query2MapList] from DB exception");
			e.printStackTrace();
		} finally {
			if (cursor != null) {
				cursor.close();
	...
	public void execSql(String sql, Object[] selectionArgs) {
		SQLiteDatabase db = null;
		//....d(TAG, "[execSql]: " + getLogSql(sql, selectionArgs));
		try {
			db = this.dbHelper.getWritableDatabase();
