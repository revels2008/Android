...
package com.jwetherell.qrcode.detector;
...
import com.jwetherell.decoder.Version;
...
public class Detector {
    ...
    public DetectorResult detect(Map<DecodeHintType, ?> hints) throws NotFoundException,
        ...
        resultPointCallback = hints == null ? null : (ResultPointCallback) hints
    ...
    protected DetectorResult processFinderPatternInfo(FinderPatternInfo info)
        ...
        if (moduleSize < 1.0f) { throw NotFoundException.getNotFoundInstance(); }
    ...
    public static PerspectiveTransform createTransform(ResultPoint topLeft, ResultPoint topRight,
    ...
    private static BitMatrix sampleGrid(BitMatrix image, PerspectiveTransform transform,
     ...
     * </p>
     */
    protected static int computeDimension(ResultPoint topLeft, ResultPoint topRight,
            ResultPoint bottomLeft, float moduleSize) throws NotFoundException {
     ...
     * </p>
     */
    protected float calculateModuleSize(ResultPoint topLeft, ResultPoint topRight,
    ...
    private float calculateModuleSizeOneWay(ResultPoint pattern, ResultPoint otherPattern) {
        float moduleSizeEst1 = sizeOfBlackWhiteBlackRunBothWays((int) pattern.getX(),
                (int) pattern.getY(), (int) otherPattern.getX(), (int) otherPattern.getY());
        float moduleSizeEst2 = sizeOfBlackWhiteBlackRunBothWays((int) otherPattern.getX(),
                (int) otherPattern.getY(), (int) pattern.getX(), (int) pattern.getY());
        if (Float.isNaN(moduleSizeEst1)) { return moduleSizeEst2 / 7.0f; }
        if (Float.isNaN(moduleSizeEst2)) { return moduleSizeEst1 / 7.0f; }
    ...
    private float sizeOfBlackWhiteBlackRun(int fromX, int fromY, int toX, int toY) {
        ...
        // Found black-white-black; give the benefit of the doubt that the next pixel outside the
        // image
    ...
    protected AlignmentPattern findAlignmentInRegion(float overallEstModuleSize, int estAlignmentX,
            int estAlignmentY, float allowanceFactor) throws NotFoundException {
        // Look for an alignment pattern (3 modules in size) around where it
        // should be
        int allowance = (int) (allowanceFactor * overallEstModuleSize);
        int alignmentAreaLeftX = Math.max(0, estAlignmentX - allowance);
        int alignmentAreaRightX = Math.min(image.getWidth() - 1, estAlignmentX + allowance);
        if (alignmentAreaRightX - alignmentAreaLeftX < overallEstModuleSize * 3) { throw NotFoundException
                .getNotFoundInstance(); }

        int alignmentAreaTopY = Math.max(0, estAlignmentY - allowance);
        int alignmentAreaBottomY = Math.min(image.getHeight() - 1, estAlignmentY + allowance);
        if (alignmentAreaBottomY - alignmentAreaTopY < overallEstModuleSize * 3) { throw NotFoundException
                .getNotFoundInstance(); }

        AlignmentPatternFinder alignmentFinder = new AlignmentPatternFinder(image,
                alignmentAreaLeftX, alignmentAreaTopY, alignmentAreaRightX - alignmentAreaLeftX,
                alignmentAreaBottomY - alignmentAreaTopY, overallEstModuleSize, resultPointCallback);
