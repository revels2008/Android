...
public class DomainGroupMoreDetail {
	public static List<DomainGroupMoreDetail> convertJson2GroupMoreDetailList(String json){
		List<DomainGroupMoreDetail> list = new ArrayList<DomainGroupMoreDetail>();
		try {
			JSONObject jsObj = new JSONObject(json);
			if(jsObj.getInt("count")>0){
				JSONArray jsArr = jsObj.getJSONArray("itemList");
				for(int i=0;i<jsArr.length();i++){
					DomainGroupMoreDetail groupMoreDetail = new DomainGroupMoreDetail();
